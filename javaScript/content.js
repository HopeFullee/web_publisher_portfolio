...
function animate() {
  ...
  let position =
    Math.floor(ImageFrame / staggerFrames) %
    spriteAnimations[currState].loc.length;
...
function charLog(text, time) {
  ...
  if (charBox) {
    // if CHAT BOX is deleted then don't spawn TEXT = (equivalent to 'clearTimeout')
    setTimeout(() => {
...
function clearLog(e) {
  ...
  if (e.target.classList.contains("user-text")) {
    ...
    while (userBox.firstChild) {
      // delete all user-text inside user-box
...
function select_2() {
  userBox.onclick = (e) => {
    ...
    if (e.target.classList.contains("A")) {
      ...
      charLog("궁굼하신 것들 무엇이든 물어봐 주세요!", "1000");
  ...
  userBox.onclick = (e) => {
    ...
    if (e.target.classList.contains("user-text")) {
      if (canvas.classList.contains("hide")) {
        // operate only when canvas has .hide from select_4 LINE:363
        const monitor = document.querySelector(".monitor");
...
function select_4() {
  userBox.onclick = (e) => {
    ...
    if (e.target.classList.contains("A")) {
      ...
      textBox();
      charLog("FE 개발자를 꿈꾸는 이소망입니다.");
      charLog("나이는 1993년 닭띠입니다.", "1000");
      ...
      charLog("이제야.. 저를 불러주시는군요.. 쿨럭..", "1200");
      charLog("한참 동안 기다리느라.. 많이 늙었지만..", "2400");
      charLog("그래도.. 저의 두 눈을 보십시오..!", "3600");
...
function select_5() {
  userBox.onclick = (e) => {
    ...
    if (!canvas.classList.contains("hide")) {
      // hotFix for broken stateTime due to canvas missing from select_4 LINE:350
    ...
    if (e.target.classList.contains("A")) {
      ...
      charLog("2002년도 10살에 사이판으로 이민간 후", "1200");
      charLog("2009년 말 귀국하였고.", "2400");
      charLog("현재는 화성시 동탄에서 거주하고 있습니다.", "3600");
      ...
      charLog("기껏해야.. 5개월치 디퍼블 국비 공장 코스..", "2000");
      ...
      textBox();
      charLog("열정과 패기로 빛나는 눈알이 두 개지요??");
      charLog("저를 불러주신다면 앞으로 더욱 빛날 것이고", "1200");
      charLog("귀사에서 빛나는 인재로써 이바지하겠습니다!", "2400");
      charLog("우선 면도 좀 하고 공중제비 한번 돌겠습니다!", "3600");
...
function select_6() {
  userBox.onclick = (e) => {
    ...
    if (e.target.classList.contains("A")) {
      ...
      charLog("방문해 주셔서 다시 한번 감사드리고,", "2400");
      charLog("귀사의 무궁한 발전을 기원합니다!", "3600");
...
function select_7() {
  userBox.onclick = (e) => {
    ...
    if (e.target.classList.contains("A")) {
      ...
      textBox();
      charLog("현시대를 살아가는 10대 20대들처럼");
      charLog("저 또한 인생의 목표와 꿈을 찾지 못하여", "1200");
      ...
      textBox();
      charLog("저의 MBTI는 전체 인구의 3%에 해당하는");
      ...
      charLog("상상하는 모든 아이디어를 코딩으로", "1200");
      charLog("완벽히 시각화하는 것이 목표이자", "2400");
...
function select_8() {
  userBox.onclick = (e) => {
    ...
    if (e.target.classList.contains("A")) {
      ...
      charLog("주변 사람들의 이야기에 귀를 귀울이니", "1200");
      charLog("저에겐 IT(코딩)계업이 어울린다는 것을", "2400");
      charLog("추천받아 도전해 보게 되었습니다.", "3600");
      ...
      charLog("미리 안될 거라 단정 지으며 시도조차", "1200");
      charLog("하지 않고 허송세월 보냈던 과거와는 달리", "2400");
      charLog("도전하며 실천할 것입니다.", "3600");
...
function select_9() {
  userBox.onclick = (e) => {
    ...
    if (e.target.classList.contains("A")) {
      ...
      textBox();
      charLog("본격적으로 공부를 시작하게 되니 욕심이", "0");
      charLog("생기게 되었고, 현재도 지식을 채우기 위해", "1200");
      charLog("노력 중입니다. 이 계열에 몸담을 수 있는", "2400");
      ...
      textBox();
      charLog("저의 성격을 객관적으로 평가한다면,");
      charLog("알기 전엔 매우 내성적이지만,", "1200");
      charLog("친해지면 전형적인 4차원 AB형이라고 합니다.", "2400");
      ...
      charLog("저는 프론트엔드 개발자가 될 것입니다.", "3600");
...
function sliderSelector(i) {
  ...
  const counter = document.createTextNode(
    sliderPosition + " / " + (itemLength - 2),
  ); // subtract 2 (cloneNodes)'s LENGTH from LINE:685
  ...
  function sliderBtnClick(e) {
    ...
    if (elem.classList.contains("next-btn")) {
      ...
      slider[i].style.transform =
        "translateX(-" + (sliderPosition + 1) * itemWidth + "%)";
      ...
      if (sliderPosition === itemLength - 2) {
        ...
        setTimeout(() => {
          ...
          slider[i].style.transform =
            "translateX(-" + sliderPosition * itemWidth + "%)";
        }, 750);
      }
      sliderPosition++;
    }
    ...
    if (sliderCounter[i].contains(counter)) {
      // remove before (counter) from LINE:705
